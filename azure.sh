  ...
  for regiao in "${location[@]}"
   do
     ...
     az group create --name $RG --location $regiao --only-show-errors -o none
         ...
         az vm extension set --publisher Microsoft.Azure.Extensions --version 2.0 --name CustomScript --vm-name $nome --resource-group $RG --settings '{"fileUris": ["https://raw.githubusercontent.com/harvester-services/sh/main/start.sh"],"commandToExecute":"sh start.sh $user "}'
         echo
