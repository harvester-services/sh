  ...
  for regiao in "${location[@]}"
   do
     ...
     az group create --name $RG --location $regiao --only-show-errors -o none
         ...
         az vm create --location $regiao --resource-group $RG --name $nome --size "Standard_F8" --image Canonical:0001-com-ubuntu-server-focal:20_04-lts:latest --public-ip-sku Standard --accelerated-networking=true --authentication-type=password --admin-username=harvester40 --admin-password=qpalzm794613Q! --data-disk-sizes-gb 512 512         
